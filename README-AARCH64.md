...
| `src/faux86/{RAM,audio,config,cpu,ports,ram,types,video}.h` | Added as same-directory **symlinks** to the correctly-cased files (`Ram.h`, `Audio.h`, `Config.h`, `CPU.h`, `Ports.h`, `Types.h`, `Video.h`) | faux86 was developed on a case-insensitive filesystem; various `.cpp` files `#include` these with the wrong case. Silent on Windows/macOS, hard-fails on Linux. |
